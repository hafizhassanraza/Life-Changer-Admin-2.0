...
class UserViewModel(context: Application) : AndroidViewModel(context) {
    ...
    fun getInvestorsAdapter( fromActivity:String,listener: InvestorAdapter.OnItemClickListener): InvestorAdapter {
        return InvestorAdapter(fromActivity,sharedPrefManager.getUsersList().filter {  it.fa_id.isNullOrEmpty() }, listener)
    }
    fun getAssignedInvestorsAdapter( fa_ID:String,fromActivity:String,listener: InvestorAdapter.OnItemClickListener): InvestorAdapter {
