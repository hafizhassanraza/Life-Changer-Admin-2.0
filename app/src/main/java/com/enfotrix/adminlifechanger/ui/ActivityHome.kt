...
class ActivityHome : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnInvestmentReq.setOnClickListener {
            startActivity(
                Intent(
                    mContext,
                    ActivityInvestmentRequest::class.java
                )
            )
        }
        binding.btnWithdrawReq.setOnClickListener {
            startActivity(
                Intent(
                    mContext,
                    ActivityWithdrawRequest::class.java
                )
            )
        }
        binding.btnNewInvestors.setOnClickListener {
            startActivity(
                Intent(
                    mContext,
                    ActivityNewInvestorReq::class.java
                )
            )
        }
        binding.btnAgentWithdrawReq.setOnClickListener {
            startActivity(
                Intent(
                    mContext,
        ...
        binding.layInActiveInv.setOnClickListener {
            startActivity(
                Intent(
                    mContext,
                    ActivityInActiveInvestment::class.java
                )
            )
        }
        binding.layInvestorsManager.setOnClickListener {
            startActivity(
                Intent(
                    mContext,
        ...
        binding.layProfitManager.setOnClickListener {
            startActivity(
                Intent(
                    mContext,
                    ActivityAddProfit::class.java
                )
            )
        }
        binding.layAgentManager.setOnClickListener {
            startActivity(
                Intent(
                    mContext,
        ...
        binding.layAnnouncementManager.setOnClickListener {
            startActivity(
                Intent(
                    mContext,
                    ActivityAnnouncement::class.java
                )
            )
        }
        binding.layAdminAccount.setOnClickListener {
            startActivity(
                Intent(
                    mContext,
                    ActivityAccounts::class.java
                )
            )
        }
        binding.layInvStatment.setOnClickListener {
            startActivity(
                Intent(
                    mContext,
                    ActivityStatment::class.java
                )
            )
        }
                ...
                .addSnapshotListener { snapshot, firebaseFirestoreException ->
                    ...
                    snapshot?.let { task ->
                        when (collection) {
                            ...
                            constants.AGENT_EARNING_COLLECTION -> sharedPrefManager.putAgentEarningList(
                                task.documents.mapNotNull { document ->
                                    document.toObject(ModelEarning::class.java)
                                        ?.apply { docID = document.id }
                                })

                            constants.NOTIFICATION_COLLECTION -> sharedPrefManager.putNotification(
                                task.documents.mapNotNull { document ->
                                    document.toObject(NotificationModel::class.java)
                                        ?.apply { id = document.id }
                                })
                            // agent withdraw requests
                            constants.WITHDRAW_COLLECTION -> sharedPrefManager.putAgentWithdrawList(
                                task.documents.mapNotNull { document ->
                                    document.toObject(AgentWithdrawModel::class.java)
                                })

                            constants.ANNOUNCEMENT_COLLECTION -> task.documents.mapNotNull { document ->
                                announcement = document.getString("announcement").toString()
                            }
                        }
    ...
    private fun setData() {
        ...
        var newInvestorsCounter = sharedPrefManager.getUsersList()
            .count { it.status.equals(constants.INVESTOR_STATUS_PENDING) }
        val pendingInvestmentCounter =
            listTransaction.count { it.type == constants.TRANSACTION_TYPE_INVESTMENT && it.status == constants.TRANSACTION_STATUS_PENDING }
                ?.toInt() ?: 0
        val pendingWithdrawCounter =
            listTransaction.count { it.type == constants.TRANSACTION_TYPE_WITHDRAW && it.status == constants.TRANSACTION_STATUS_PENDING }
                ?.toInt() ?: 0
        ...
        var ActiveInvestment = listInvestmentModel.sumOf {
            it.investmentBalance.takeIf { it.isNotBlank() }?.toInt() ?: 0
        }.toInt()
        var Profit =
            listInvestmentModel.sumOf { it.lastProfit.takeIf { it.isNotBlank() }?.toInt() ?: 0 }
                .toInt()
        var InActiveInvestment =
            listInvestmentModel.sumOf { it.lastInvestment.takeIf { it.isNotBlank() }?.toInt() ?: 0 }
                .toInt()
        var totalSum = ActiveInvestment + InActiveInvestment + Profit

        var AgentCounter = sharedPrefManager.getFAList().count()
        var AgentWithdrawReqCounter = sharedPrefManager.getAgentWithdrawList()
            .filter { it.status.equals(constants.TRANSACTION_STATUS_PENDING) }.count()
        var ActiveInvestorCounter = sharedPrefManager.getUsersList()
            .count { it.status.equals(constants.INVESTOR_STATUS_ACTIVE) }
